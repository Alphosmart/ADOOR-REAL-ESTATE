...
const { buildPropertyFields } = require('../utils/propertyFields');
...
async function addProductController(req, res) {
    try {
        ...
        upload(req, res, async function (err) {
            // Errors thrown in this callback escape the outer try/catch and would become
            // unhandled rejections, which exit the whole server - so catch them here.
            try {
                ...
                // Listing type, status, location and property details (shared with update-product)
                const propertyFields = buildPropertyFields(req.body);
                ...
                const newProduct = new Product({
                    ...
                    sellerInfo: {
                        ...
                        currency: sellerCurrency,
                        location: currentUser.address?.country || req.body.country
                    },
                    ...
                    status: propertyFields.status || 'ACTIVE',
                    listingType: propertyFields.listingType || '',
                    location: propertyFields.location || {},
                    propertyDetails: propertyFields.propertyDetails || {},
                ...
                console.log("Error in addProduct:", saveError.message);
                return res.status(saveError.name === 'ValidationError' ? 400 : 500).json({
                    message: "Failed to add product: " + saveError.message,
                    error: true,
                    success: false
                });
            }
